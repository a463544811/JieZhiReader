...
-(void)catalogShowState:(BOOL)show
{
    show?({
        ...
        [UIView animateWithDuration:AnimationDelay animations:^{
            _catalogView.frame = CGRectMake(0, 66,2*ViewSize(self.view).width, ViewSize(self.view).height);
        ...
        [UIView animateWithDuration:AnimationDelay animations:^{
             _catalogView.frame = CGRectMake(-ViewSize(self.view).width, 66, 2*ViewSize(self.view).width, ViewSize(self.view).height);
        } completion:^(BOOL finished) {
            _catalogView.hidden = !show;
...
-(void)menuViewDirectory{
    TJZBookDirectoryViewController * vc = [TJZBookDirectoryViewController new];
...
-(void)viewDidLayoutSubviews
{
    ...
    _catalogVC.view.frame = CGRectMake(0, 0, ViewSize(self.view).width, ViewSize(self.view).height);
    [_catalogVC reload];
}
@end
