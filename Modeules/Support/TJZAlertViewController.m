...
@interface TJZAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(TJZAlertAction *action);

@end

@implementation TJZAlertAction

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(TJZAlertAction *action))handler {
    TJZAlertAction *instance = [TJZAlertAction new];
...
@interface TJZAlertViewController () {
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation TJZAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    TJZAlertViewController *instance = [TJZAlertViewController new];
...
- (void)creatAllButtons {
    for (int i=0; i<self.actions.count; i++) {
        ...
        [btn setTitleColor:[UIColor colorWithHex:@"#999999"] forState:UIControlStateNormal];
...
- (void)updateAllButtonsFrame {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        if (i==1) {
            [btn setTitleColor:[UIColor colorWithHex:@"#65c9a1"] forState:UIControlStateNormal];
        }
...
- (void)didClickButton:(UIButton *)sender {
    TJZAlertAction *action = self.actions[sender.tag-10];
...
- (NSArray<TJZAlertAction *> *)actions {
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [self creatLabelWithFontSize:19];
